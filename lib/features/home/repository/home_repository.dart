    ...
    if (!isGranted) {
      ...
      try {
        ...
        final List<Placemark> placemarks =
            await UserLocation.geoCoding(position);
  ...
  Future<List<HomeModel>> fetchAllExceptCurrentUser() async {
    try {
      ...
      final filteredDocs = snapshot.docs.where((doc) {
        ...
        if (!shouldInclude) {
          print(
              'HomeRepository: Excluding user - Document ID: $documentId, Data ID: $docId, Current UID: $uid');
        }
        return shouldInclude;
      }).toList();
      print(
          'HomeRepository: After filtering current user: ${filteredDocs.length} documents');
      print(
          'HomeRepository: After filtering current user: ${filteredDocs.length} documents');
      ...
      final String viewerProfileType =
          (currentUserModel?.isElite ?? false) ? 'Élite' : 'Classique';
      ...
      final users = filteredDocs.map((doc) {
        ...
        print(
            'HomeRepository: Has photos field: ${data.containsKey('photos')}');
        if (data.containsKey('photos')) {
          print('HomeRepository: Photos field: ${data['photos']}');
          print(
              'HomeRepository: Photos field type: ${data['photos'].runtimeType}');
          if (data['photos'] is List) {
            print(
                'HomeRepository: Photos list length: ${(data['photos'] as List).length}');
        ...
        print(
            'HomeRepository: Has gender field: ${data.containsKey('gender')}');
      ...
      if (users.isNotEmpty) {
        ...
        final firstPhoto =
            firstUser.photos.isNotEmpty ? firstUser.photos.first : 'No photo';
        print(
            'HomeRepository: First user - ID: ${firstUser.id}, Pseudo: ${firstUser.pseudo}, imgURL: $firstPhoto');
      }
      ...
      final filteredByInvisibility = users.where((user) {
        final hidesFromType =
            user.invisibleToProfileTypes.contains(viewerProfileType);
        final hidesFromSex = user.invisibleToSexes.contains(viewerSex);
        final hidesFromOrientation =
            user.invisibleToOrientations.contains(viewerOrientation);
        final isInvisibleToViewer =
            hidesFromType || hidesFromSex || hidesFromOrientation;
      ...
      for (final user in filteredByInvisibility) {
        ...
        if (user.id == uid) {
          print(
              'HomeRepository: Excluding current user from results - User ID: ${user.id}, Current UID: $uid');
          continue;
        }

        final isInvisible =
            await _invisibleModeService.isUserInvisible(user.id);
        if (!isInvisible) {
          ...
          final hasInteracted =
              await _invisibleModeService.hasInteractedWith(user.id);
          if (hasInteracted) {
      ...
      final finalFiltered = filteredByInvisibleMode.where((user) {
        ...
        if (isCurrentUser) {
          print(
              'HomeRepository: Final filter - Excluding current user: ${user.id}');
        }
  ...
  Future<List<HomeModel>> findUsersForBlock({
    ...
    try {
      ...
      final others = snapshot.docs
          .where((doc) => (doc.data()['id'] as String? ?? '') != uid)
          .toList();

      final Set<String> profileTypes =
          selectedProfileTypes.map((e) => e).toSet();
      final Set<String> sexes = selectedSexes.map((e) => e).toSet();
      final Set<String> orientations =
          selectedOrientations.map((e) => e).toSet();
      ...
      for (final d in others) {
        ...
        if (profileTypes.isNotEmpty && !profileTypes.contains(userType))
          continue;
        ...
        // orientation
        if (orientations.isNotEmpty && !orientations.contains(hm.orientation))
          continue;

        // age
        final int age = (data['age'] as num?)?.toInt() ?? 0;
        if (ageRange.start > 0 || ageRange.end > 0) {
          if (!(age >= ageRange.start.round() && age <= ageRange.end.round()))
            continue;
        }

        // marital status (optional)
        if (selectedMaritalStatuses != null &&
            selectedMaritalStatuses.isNotEmpty) {
        ...
        if (selectedMainInterests != null && selectedMainInterests.isNotEmpty) {
          final List<dynamic> mi =
              (data['mainInterests'] as List<dynamic>?) ?? const [];
          final Set<String> miSet = mi.map((e) => e.toString()).toSet();
          if (miSet.intersection(selectedMainInterests.toSet()).isEmpty)
            continue;
        }
